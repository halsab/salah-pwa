...
export interface ScheduleDiagnostic {
  code: 'ambiguous-date' | 'unusual-order' | 'equal-times' | 'midnight-transition' | 'day-jump'
  locationId: string
  date: string
  fields: PrayerKey[]
  reason: string
...
export function diagnoseOfficialSchedule(days: readonly PrayerDay[]): ScheduleDiagnostic[] {
  ...
  for (const day of sorted) {
    ...
    if (minutes(day.suhurEnd) >= 12 * 60) {
      warn('ambiguous-date', ['suhurEnd'], `Сухур ${day.suhurEnd}: источник не уточняет календарную дату около полуночи; значение сохранено, таймер исключён.`)
    }
