...
export interface ScheduleDiagnostic {
  code: 'previous-day-fajr-start' | 'unusual-order' | 'equal-times' | 'midnight-transition' | 'day-jump'
  locationId: string
  date: string
  fields: PrayerKey[]
  reason: string
...
export function diagnoseOfficialSchedule(days: readonly PrayerDay[]): ScheduleDiagnostic[] {
  ...
  for (const day of sorted) {
    ...
    if (minutes(day.fajrStart) >= 12 * 60) {
    ...
    const expectedOrder: readonly (readonly [PrayerKey, PrayerKey])[] = [
      ['fajrStart', 'fajrJamaat'], ['fajrJamaat', 'sunrise'], ['sunrise', 'zenith'],
