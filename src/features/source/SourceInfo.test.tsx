...
it('показывает факты официальной таблицы и дату позднего сухура без раскрытия идентификаторов', async () => {
  ...
  await userEvent.click(within(dialog).getByText('Подробности'))
...
it('расчёт показывает эффективные параметры, ручные поправки и приблизительность', async () => {
