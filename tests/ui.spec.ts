...
import { back, choosePlace, openSchedule, openSource, setSource, expect, test } from './fixtures'
...
for (const viewport of [{ width: 320, height: 640 }, { width: 390, height: 844 }, { width: 844, height: 390 }, { width: 1280, height: 900 }]) {
  test(`${viewport.width}×${viewport.height}: экраны, поля 16, единая типографика и доступные действия`, async ({ page }) => {
    ...
    await expect(page.getByRole('region', { name: 'Текущее событие' })).toContainText('Зухр')
    await geometry(page)
...
test('календарь открывает выбранный день, возврат восстанавливает сегодня и фокус', async ({ page }) => {
  await page.goto('./')
  await choosePlace(page)
  await page.getByLabel('Выбрать дату').fill('2026-09-01')
  await expect(page.getByRole('region', { name: 'Расписание', exact: true })).toBeVisible()
